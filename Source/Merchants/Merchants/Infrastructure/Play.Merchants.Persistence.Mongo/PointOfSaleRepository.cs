...
internal class PointOfSaleRepository : IPointOfSaleRepository
{
    ...
    {
        IEnumerable<PointOfSaleConfiguration> result =
            await _MongoDbHelper.SelectAsync<PointOfSaleConfiguration>(_CollectionName, filter => filter.TerminalId == terminalId);
    ...
    {
        long count = await _MongoDbHelper.CountAsync<PointOfSaleConfiguration>(_CollectionName, filter => filter.Id == id);
