...
public abstract record BrokenRuleOrPolicyDomainEvent<_Aggregate, _TId> : DomainEvent where _Aggregate : Aggregate<_TId> where _TId : IEquatable<_TId>
{
    #region Constructor

    protected BrokenRuleOrPolicyDomainEvent(_Aggregate aggregate, IBusinessRule rule) : base($"{GetRuleDescription(aggregate, rule)}")
    { }
