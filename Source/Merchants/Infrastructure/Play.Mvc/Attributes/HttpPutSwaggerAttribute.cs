...
public class HttpPutSwaggerAttribute : HttpPutAttribute
{
    ...
    public HttpPutSwaggerAttribute([CallerFilePath] string callerFilePath = "", [CallerMemberName] string memberName = "")
    {
        ...
        {
            Name = $"{FormatControllerName(Path.GetFileNameWithoutExtension(callerFilePath))}_{memberName}_Put";

            return;
        }

        Name = $"{areasName}_{FormatControllerName(Path.GetFileNameWithoutExtension(callerFilePath))}_{memberName}_Put";
    }
