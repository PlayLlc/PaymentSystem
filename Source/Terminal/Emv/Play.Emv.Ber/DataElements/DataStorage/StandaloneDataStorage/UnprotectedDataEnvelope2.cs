...
public record UnprotectedDataEnvelope2 : DataElement<BigInteger>, IEqualityComparer<UnprotectedDataEnvelope2>
{
    ...
    public static UnprotectedDataEnvelope2 Decode(ReadOnlyMemory<byte> value) => Decode(value.Span);

    public override UnprotectedDataEnvelope2 Decode(TagLengthValue value) => Decode(value.EncodeValue().AsSpan());

    /// <exception cref="DataElementParsingException"></exception>
    /// <exception cref="CodecParsingException"></exception>
    public static UnprotectedDataEnvelope2 Decode(ReadOnlySpan<byte> value)
    {
        Check.Primitive.ForMaximumLength(value, _MaxByteLength, Tag);

        BigInteger result = PlayCodec.BinaryCodec.DecodeToBigInteger(value);

        return new UnprotectedDataEnvelope2(result);
    }
