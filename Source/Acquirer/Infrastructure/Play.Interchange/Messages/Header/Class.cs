...
public sealed record Class : EnumObject<byte>
{
    ...
    private static readonly Dictionary<byte, Class> _ValueObjectMap = new()
    {
    ...
    public override Class[] GetAll() => _ValueObjectMap.Values.ToArray();

    public override bool TryGet(byte value, out EnumObject<byte>? result)
    {
        if (_ValueObjectMap.TryGetValue(value, out Class? enumResult))
        {
            result = enumResult;
