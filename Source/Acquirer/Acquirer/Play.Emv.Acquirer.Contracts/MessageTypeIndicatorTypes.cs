...
internal sealed record MessageTypeIndicatorTypes : EnumObject<ushort>
{
    ...
    public override MessageTypeIndicatorTypes[] GetAll() => _ValueObjectMap.Values.ToArray();

    public override bool TryGet(ushort value, out EnumObject<ushort>? result)
    {
        if (_ValueObjectMap.TryGetValue(value, out MessageTypeIndicatorTypes? enumResult))
        {
            result = enumResult;
    ...
    public static implicit operator MessageTypeIndicator(MessageTypeIndicatorTypes value) => value._Value;
