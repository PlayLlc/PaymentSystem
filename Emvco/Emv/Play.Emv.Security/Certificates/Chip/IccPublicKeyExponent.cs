...
public record IccPublicKeyExponent : PrimitiveValue, IEqualityComparer<IccPublicKeyExponent>
{
    ...
    public static IccPublicKeyExponent Decode(ReadOnlySpan<byte> value, BerCodec codec)
    {
        ...
        DecodedResult<uint> result = codec.Decode(BerEncodingId, value) as DecodedResult<uint>
            ?? throw new
                InvalidOperationException($"The {nameof(IccPublicKeyExponent)} could not be initialized because the {nameof(UnsignedBinaryCodec)} returned a null {nameof(DecodedResult<uint>)}");
