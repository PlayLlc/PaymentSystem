...
public record CardholderVerificationCode3Track2 : PrimitiveValue, IEqualityComparer<CardholderVerificationCode3Track2>
{
    ...
    public static CardholderVerificationCode3Track2 Decode(ReadOnlySpan<byte> value, BerCodec codec)
    {
        ...
        DecodedResult<ushort> result = codec.Decode(BerEncodingId, value) as DecodedResult<ushort>
            ?? throw new
                InvalidOperationException($"The {nameof(CardholderVerificationCode3Track2)} could not be initialized because the {nameof(UnsignedBinaryCodec)} returned a null {nameof(DecodedResult<ushort>)}");
