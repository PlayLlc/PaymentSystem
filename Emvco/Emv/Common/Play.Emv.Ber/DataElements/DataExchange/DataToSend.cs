...
public sealed record DataToSend : DataExchangeResponse, IEqualityComparer<DataToSend>
{
    ...
    public static DataToSend Decode(ReadOnlyMemory<byte> value) => new(Codec.DecodePrimitiveValuesAtRuntime(value.Span).ToArray());

    /// <exception cref="BerParsingException"></exception>
    public static DataToSend Decode(ReadOnlySpan<byte> value) => new(Codec.DecodePrimitiveValuesAtRuntime(value.ToArray()).ToArray());
