...
public record OfflineAccumulatorBalance : PrimitiveValue, IEqualityComparer<OfflineAccumulatorBalance>
{
    ...
    public static OfflineAccumulatorBalance Decode(ReadOnlySpan<byte> value, BerCodec codec)
    {
        ...
        DecodedResult<ulong> result = codec.Decode(BerEncodingId, value) as DecodedResult<ulong>
            ?? throw new
                InvalidOperationException($"The {nameof(OfflineAccumulatorBalance)} could not be initialized because the {nameof(BinaryCodec)} returned a null {nameof(DecodedResult<ulong>)}");
