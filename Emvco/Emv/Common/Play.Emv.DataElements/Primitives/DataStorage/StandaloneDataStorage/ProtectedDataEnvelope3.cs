...
public record ProtectedDataEnvelope3 : DataElement<BigInteger>, IEqualityComparer<ProtectedDataEnvelope3>
{
    ...
    public static ProtectedDataEnvelope3 Decode(ReadOnlySpan<byte> value, BerCodec codec)
    {
        ...
        DecodedResult<BigInteger> result = codec.Decode(BerEncodingId, value) as DecodedResult<BigInteger>
            ?? throw new
                InvalidOperationException($"The {nameof(ProtectedDataEnvelope3)} could not be initialized because the {nameof(BinaryCodec)} returned a null {nameof(DecodedResult<BigInteger>)}");
