...
public record UnprotectedDataEnvelope3 : DataElement<BigInteger>, IEqualityComparer<UnprotectedDataEnvelope3>
{
    ...
    public static UnprotectedDataEnvelope3 Decode(ReadOnlySpan<byte> value, BerCodec codec)
    {
        ...
        DecodedResult<BigInteger> result = codec.Decode(BerEncodingId, value) as DecodedResult<BigInteger>
            ?? throw new
                InvalidOperationException($"The {nameof(UnprotectedDataEnvelope3)} could not be initialized because the {nameof(BinaryCodec)} returned a null {nameof(DecodedResult<BigInteger>)}");
