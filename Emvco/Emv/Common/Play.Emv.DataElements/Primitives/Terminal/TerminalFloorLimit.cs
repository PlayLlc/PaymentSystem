...
public record TerminalFloorLimit : DataElement<uint>, IEqualityComparer<TerminalFloorLimit>
{
    ...
    public static TerminalFloorLimit Decode(ReadOnlySpan<byte> value)
    {
        ...
        DecodedResult<uint> result = _Codec.Decode(BerEncodingId, value) as DecodedResult<uint>
            ?? throw new
                InvalidOperationException($"The {nameof(TerminalFloorLimit)} could not be initialized because the {nameof(BinaryCodec)} returned a null {nameof(DecodedResult<uint>)}");
