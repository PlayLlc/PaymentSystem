...
public record AdditionalTerminalCapabilities : DataElement<ulong>, IEqualityComparer<AdditionalTerminalCapabilities>
{
    ...
    public static AdditionalTerminalCapabilities Decode(ReadOnlySpan<byte> value)
    {
        ...
        DecodedResult<ulong> result = _Codec.Decode(BerEncodingId, value) as DecodedResult<ulong>
            ?? throw new
                InvalidOperationException($"The {nameof(AdditionalTerminalCapabilities)} could not be initialized because the {nameof(BinaryCodec)} returned a null {nameof(DecodedResult<ulong>)}");
