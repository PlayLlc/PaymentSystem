...
internal partial class S910
{
    private partial class AuthHandler
    {
        ...
        public StateId ProcessWithoutCda(IGetKernelStateId currentStateIdRetriever, Kernel2Session session, GenerateApplicationCryptogramResponse rapdu)
        {
            // S910.30 - S910.31
            if (TryHandlingForMissingMandatoryData(session.GetKernelSessionId()))
                return currentStateIdRetriever.GetStateId();

            return IsApplicationAuthenticationCryptogram() ? HandleAac(currentStateIdRetriever, session) : HandleIsNotAac(currentStateIdRetriever, session);
        }
