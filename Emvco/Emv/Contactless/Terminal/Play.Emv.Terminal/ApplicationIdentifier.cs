...
public record ApplicationIdentifier : PrimitiveValue, IEqualityComparer<ApplicationIdentifier>
{
    ...
    public static ApplicationIdentifier Decode(ReadOnlySpan<byte> value, BerCodec codec)
    {
        ...
        {
            throw new
                ArgumentOutOfRangeException($"The Primitive Value {nameof(ApplicationIdentifier)} could not be initialized because the byte length provided was out of range. The byte length was {value.Length} but must be in the range of {minByteLength}-{maxByteLength}");
        }
