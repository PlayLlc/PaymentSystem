    ...
    IEqualityComparer<UserInterfaceRequestData>
{
    ...
    public static UserInterfaceRequestData Decode(ReadOnlySpan<byte> value)
    {
        ...
        DecodedResult<BigInteger> result = _Codec.Decode(BerEncodingId, value) as DecodedResult<BigInteger>
            ?? throw new
                InvalidOperationException($"The {nameof(UserInterfaceRequestData)} could not be initialized because the {nameof(UnsignedBinaryCodec)} returned a null {nameof(DecodedResult<BigInteger>)}");
