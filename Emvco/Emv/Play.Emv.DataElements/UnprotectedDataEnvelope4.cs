...
public record UnprotectedDataEnvelope4 : DataElement<BigInteger>, IEqualityComparer<UnprotectedDataEnvelope4>
{
    ...
    public static UnprotectedDataEnvelope4 Decode(ReadOnlySpan<byte> value, BerCodec codec)
    {
        ...
        {
            throw new
                ArgumentOutOfRangeException($"The Primitive Value {nameof(UnprotectedDataEnvelope4)} could not be initialized because the byte length provided was out of range. The byte length was {value.Length} but must be less than {maxByteLength} bytes in length");
        }

        DecodedResult<BigInteger> result = codec.Decode(BerEncodingId, value) as DecodedResult<BigInteger>
            ?? throw new
                InvalidOperationException($"The {nameof(UnprotectedDataEnvelope4)} could not be initialized because the {nameof(UnsignedBinaryCodec)} returned a null {nameof(DecodedResult<BigInteger>)}");
