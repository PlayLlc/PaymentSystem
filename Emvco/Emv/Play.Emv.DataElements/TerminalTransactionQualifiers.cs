...
public record TerminalTransactionQualifiers : DataElement<uint>, IEqualityComparer<TerminalTransactionQualifiers>
{
    ...
    public static TerminalTransactionQualifiers Decode(ReadOnlySpan<byte> value)
    {
        ...
        {
            throw new
                ArgumentOutOfRangeException($"The Primitive Value {nameof(TerminalTransactionQualifiers)} could not be initialized because the byte length provided was out of range. The byte length was {value.Length} but must be {_ByteLength} bytes in length");
        }

        DecodedResult<uint> result = _Codec.Decode(BerEncodingId, value) as DecodedResult<uint>
            ?? throw new
                InvalidOperationException($"The {nameof(TerminalTransactionQualifiers)} could not be initialized because the {nameof(UnsignedBinaryCodec)} returned a null {nameof(DecodedResult<uint>)}");
