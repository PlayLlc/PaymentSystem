...
public record ApplicationInterchangeProfile : DataElement<ushort>, IEqualityComparer<ApplicationInterchangeProfile>
{
    ...
    public static ApplicationInterchangeProfile Decode(ReadOnlySpan<byte> value)
    {
        ...
        {
            throw new
                ArgumentOutOfRangeException($"The Primitive Value {nameof(ApplicationInterchangeProfile)} could not be initialized because the byte length provided was out of range. The byte length was {value.Length} but must be {byteLength} bytes in length");
        }
